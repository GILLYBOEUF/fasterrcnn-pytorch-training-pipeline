...
# training with Keypoint RCNN model with a Faster RCNN ResNet50 FPN backbone trained for lower body detection without mosaic or any other augmentation:
python train.py --model keypointrcnn_resnet50_fpn --epochs 2 --data data_configs/keypoints_data.yaml --no-mosaic --batch 4
...
from torch.utils.data import (
    distributed, RandomSampler, SequentialSampler, DataLoader
)
...
def main(args):
    ...
    KEYPOINTS_FOLDER_TRAIN = os.path.normpath(data_configs['KEYPOINTS_FOLDER_TRAIN'])
    KEYPOINTS_FOLDER_VAL = os.path.normpath(data_configs['KEYPOINTS_FOLDER_VAL'])
    # KEYPOINTS_FOLDER_TEST = os.path.normpath(data_configs['KEYPOINTS_FOLDER_TEST'])
    CLASSES = data_configs['CLASSES']
    NUM_CLASSES = data_configs['NC']
    NUM_KEYPOINTS = data_configs['NK']
